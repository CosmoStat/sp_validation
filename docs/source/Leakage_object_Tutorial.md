...
**Output** : 
	* For `--PSF_Leakage` -> Plots of the 2d Linear and quadratic Fits e_gal vs e_PSF 
...
![](Leakage_object_Tutorial/PSF_Leakage2.png)
	...
	* For `--Obs_Leakage` -> Plots 1d Linear or quadratic fits of e_gal vs quantities
...
(with `--ratio` and `--linear options`) 			   (with `--quadratic option`)
