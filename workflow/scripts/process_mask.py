...
def load_healsparse_mask(mask_path: str, verbose: bool = True) -> hsp.HealSparseMap:
    ...
    if verbose:
        print(
            f"Loaded mask with nside_coverage={mask.nside_coverage}, nside_sparse={mask.nside_sparse}"
        )
...
def degrade_mask(
    mask: hsp.HealSparseMap, target_nside: int, verbose: bool = True
) -> np.ndarray:
            ...
            "masks": {
                f"nside_{nside}": {
                    ...
                    "coverage_fraction": float(
                        np.sum(healpix_mask > 0) / len(healpix_mask)
                    ),
                    "n_valid_pixels": int(np.sum(healpix_mask > 0)),
                    "total_pixels": int(len(healpix_mask)),
                    "mean_mask_value": float(
                        np.mean(healpix_mask[healpix_mask > 0])
                        if np.sum(healpix_mask > 0) > 0
                        else 0.0
                    ),
