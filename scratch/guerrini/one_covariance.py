...
# %%
redshift_distribution = np.loadtxt(
    "/n17data/mkilbing/astro/data/CFIS/v1.0/nz/dndz_SP_A.txt"
)
...
np.savetxt(
    "/home/guerrini/OneCovariance/input/redshift_distribution/dndz_SP_A.asc",
...
# %%
shape_noise = 0.5 * (
    np.sum(cat_gal["w"] ** 2 * cat_gal["g1"] ** 2) / np.sum(cat_gal["w"] ** 2)
    + np.sum(cat_gal["w"] ** 2 * cat_gal["g2"] ** 2) / np.sum(cat_gal["w"] ** 2)
)
np.sqrt(shape_noise)
...
# %%
hp.write_map(
    "/home/guerrini/OneCovariance/input/mask/healpix_mask_SPv1.fits",
    mask,
    overwrite=True,
)
...
step = len(bins_ell) // 4
plt.xticks(
    ticks=np.arange(0, len(bins_ell), step), labels=np.round(bins_ell[::step], 1)
)
plt.yticks(
    ticks=np.arange(0, len(bins_ell), step), labels=np.round(bins_ell[::step], 1)
)
...
plt.plot(
    bins_ell,
    np.diag(cov_matrix_one_cov_gaussian),
    label="Gaussian Covariance (OneCovariance)",
)
plt.plot(
    bins_ell, np.diag(cov_matrix_one_cov_pure), label="Pure Covariance (OneCovariance)"
)
...
fiducial_cl = np.array(
    [fiducial_cl, 0.0 * fiducial_cl, 0.0 * fiducial_cl, 0.0 * fiducial_cl]
...
covar_22_22 = nmt.gaussian_covariance(
    cw, 2, 2, 2, 2, fiducial_cl, fiducial_cl, fiducial_cl, fiducial_cl, wsp, wb=wsp
).reshape([32, 4, 32, 4])
...
cov_cosmo_val_145 = fits.open("./output/pseudo_cl_cov_SP_v1.4.5.fits")[
    "COVAR_EE_EE"
].data

plt.plot(bins_ell_mid, np.diag(cov_cosmo_val_145), label="SP_v1.4.5")
plt.plot(
    bins_ell_mid, np.diag(covar_22_22[:, 0, :, 0]), label="SP_v1.4.5 w mask SP_v1.4.5.A"
)
