...
# %%
best_fit_axel = np.load(
    "/n17data/guinot/CFIS_3500/CFIS_1.4.5/inference/plots_nautilus_SP_v1_full_model_no_B_IA_opti_scale/best_model_cell_nautilus_SP_v1_full_model_no_B_IA_opti_scale.npy"
)
...
ax.errorbar(
    ell_eff,
    ell_eff * pseudo_cl_glass["EE"],
    yerr=ell_eff * np.sqrt(np.diag(cov_cl_glass["COVAR_EE_EE"].data)),
    fmt="o",
    capsize=2,
    markersize=4,
    label="EE",
...
ax.errorbar(
    ell_eff,
    ell_eff * pseudo_cl_glass["BB"],
    yerr=ell_eff * np.sqrt(np.diag(cov_cl_glass["COVAR_BB_BB"].data)),
    fmt="o",
    capsize=2,
    markersize=4,
    label="BB",
...
data_vector = fits.getdata(
    cosmo_val_outdir + "/pseudo_cl_SP_v1.4.6.3_B_leak_corr.fits"
)["BB"][12:-3]
cov = fits.open(cosmo_val_outdir + "/pseudo_cl_cov_SP_v1.4.6.3_B_leak_corr.fits")[
    "COVAR_BB_BB"
].data[12:-3, 12:-3]

# %%
path_gaussian_sims = (
    "/home/guerrini/sp_validation/cosmo_val/harmonic_covariance_gaussian_sims_v1463_B/"
)
for i in range(10_000):
    sim_cl = np.load(path_gaussian_sims + f"sample_{i}.npz")
    if i == 0:
...
# %%
data_vector_gaussian = fits.getdata(
    cosmo_val_outdir + "/pseudo_cl_SP_v1.4.6.3_A_leak_corr.fits"
)["EB"]
...
plt.errorbar(
    ell,
    ell * data_vector,
    yerr=ell * np.sqrt(np.diag(cov_141)),
    fmt="o",
    label="SP_v1.4.1",
    capsize=2,
)
plt.errorbar(
    ell,
    ell * data_vector_sim[3],
    yerr=ell * np.sqrt(np.diag(cov_141_sim[3])),
    fmt="o",
    label="SP_v1.4.1_sim",
    capsize=2,
)
