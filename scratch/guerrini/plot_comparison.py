...
n_eff_gal = (
    1 / (area * 60 * 60) * (np.sum(cat_gal["w_iv"])) ** 2 / np.sum(cat_gal["w_iv"] ** 2)
)
...
sigma_e = np.sqrt(
    0.5
    * (
        np.sum((cat_gal["e1"] * cat_gal["w_iv"]) ** 2)
        + np.sum((cat_gal["e2"] * cat_gal["w_iv"]) ** 2)
    )
    / np.sum(cat_gal["w_iv"] ** 2)
)
...
psf_fitter = PSFErrorFit(
    rho_stats_handler, tau_stats_handler, root_dir + "/rho_tau_stats/"
)
...
for ver, color in zip(versions, colors):
    ...
    plt.fill_between(
        theta, xi_psf_sys_quant[0], xi_psf_sys_quant[1], color=color, alpha=0.1
    )
...
plt.plot(
    ...
    c="black",
    ls="--",
    label=str(threshold) + "X DES level",
)
plt.plot([0.1, 250], [-threshold, -threshold], c="k", ls="--")
plt.xscale("log")
plt.xlabel(r"$\theta$ (arcmin)")
...
for ver, color in zip(versions, colors):
    ...
    gg.read(root_dir + "/xi_pm_" + ver + ".txt")
    plt.errorbar(
        gg.meanr, gg.xip, yerr=np.sqrt(gg.varxip), label=ver, color=color, capsize=2
    )
plt.xscale("log")
plt.yscale("log")
plt.xlabel(r"$\theta$ (arcmin)")
...
cat_gal = fits.getdata(
    "/n17data/mkilbing/astro/data/CFIS/v1.0/ShapePipe/unions_shapepipe_2024_v1.4.1.fits"
)
...
for ver, color in zip(versions, colors):
    ...
    plt.errorbar(
        theta_map,
        mapsq,
        yerr=np.sqrt(varmapsq),
        c=color,
        label=r"$\langle M^2_{\rm ap} \rangle$ " + ver,
        capsize=2,
    )
    plt.errorbar(
        theta_map,
        ...
        c=color,
        ls="--",
        label=r"$\langle M^2_\times \rangle$ " + ver,
        capsize=2,
    )
