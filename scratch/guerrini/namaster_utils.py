...
def load_cat_des(path_cat):
    ...
    e1 = (e1 - np.average(e1, weights=weight)) / np.average(
        cat_gal["R11"], weights=weight
    )
    e2 = (e2 - np.average(e2, weights=weight)) / np.average(
        cat_gal["R22"], weights=weight
    )
...
def load_cl_measurement(path_cl):
    ...
    cl_array = np.array(
        [cl_data["EE"].data, cl_data["EB"].data, cl_data["EB"].data, cl_data["BB"].data]
    )
    return cl_array
...
def get_theory(lmax, z, nz):
    ...
    cl_theo = get_theo_c_ell(ell=ell, z=z, nz=nz, backend="camb", cosmo=get_cosmo())
    return cl_theo
...
def get_covariance(cl_array, cw, wsp, num_bins):
    covar_22_22 = nmt.gaussian_covariance(
        cw, 2, 2, 2, 2, cl_array, cl_array, cl_array, cl_array, wsp, wb=wsp
...
def get_covariance_from_glass(root, n_sim, num_bins, lmax, type):
    if type == "cat":
        ...
        raise ValueError(
            "Type must be one of 'cat', 'cat_coupled', 'map', 'map_coupled'"
        )
