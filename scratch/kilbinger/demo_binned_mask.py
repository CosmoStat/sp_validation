...
masks, labels = sp_joint.get_masks_from_config(
    config, dat, dat_ext, masks_to_apply=masks_to_apply, verbose=True
)
    ...
    for mask in masks:
        if mask._col_name != label_base_mask:
            ...
            print(
                ...
                "#True = ",
                sum(mask._mask),
                f"({100 * sum(mask._mask) / len(mask._mask):.2f}%)",
            )
else:
...
nsides = np.array(
    [2**i for i in range(int(np.log2(nside_min)), int(np.log2(nside_max)) + 1)]
)
