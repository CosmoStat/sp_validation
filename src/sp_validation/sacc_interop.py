...
:Name: sacc_interop.py

:Description: Converters between the SACC data product
              (:mod:`sp_validation.sacc_io`) and external analysis-tool file
              formats — the CosmoSIS "2pt FITS" and OneCovariance's redshift /
              covariance files. Two independent, self-contained sections; the
              only shared surface is the SACC on one side.

              **CosmoSIS 2pt-FITS** (:func:`sacc_to_twopoint_fits`). Convert an
              analysis SACC into the "2pt FITS" that CosmoSIS's ``2pt_like`` (and
              Sacha Guerrini's ρ/τ ``2pt_like_xi_sys`` fork) reads. The output
              reproduces today's hand-assembled product from
              ``cosmo_inference/scripts/cosmosis_fitting.py`` HDU-for-HDU and
              byte-for-byte (verified against that writer): an NZDATA table,
              XI_PLUS / XI_MINUS 2pt tables, optional CELL_EE / CELL_BB pseudo-Cℓ
              tables, the blocked COVMAT (with ``STRT_i`` block-offset headers)
              and separate COVMAT_CELL, and — when the SACC carries them — the
              TAU_{0,2}_PLUS 2pt tables and the RHO_STATS table.
              ...
              ``BIN1`` column. The converter reads bin ``(0, 0)`` accordingly and
              fails fast on a multi-bin SACC (single-bin v1 contract). A
              tomographic 2pt-FITS layout (multiple bin pairs, per-pair
              ...
              **OneCovariance** (:func:`write_nz`, :func:`nz_config_stanza`,
              :func:`read_nz`, :func:`covariance_blocks`). File-format glue
              between the SACC layout and OneCovariance
              (https://github.com/rreischke/OneCovariance). Two directions: the
              ``source_i`` NZ tracers are written as the combined
              whitespace-delimited redshift file OneCovariance reads (column 0 =
              z grid, then one ``n(z)`` column per bin, no bin edges) with a
              matching ``[redshift]`` config stanza; and the flat
              ``covariance_list_*.dat`` table OneCovariance emits is reshaped
              into dense SACC covariance block(s) paired with their selectors.
              ...
              ``z_high`` edges (distinct from the CosmoSIS NZDATA table above,
              which *does* carry edges). All source bins must share one z grid.
"""
