...
The converter (:mod:`sp_validation.sacc_io`) must reproduce the CosmoSIS
...
def _sacc(inp, *, cl=False, rho_tau=False):
    ...
    sacc_io.add_xi(s, (0, 0), inp["theta"], inp["xip"], inp["xim"], grid="reporting")
    if cl:
...
def test_plain_xi_byte_equal(tmp_path):
    ...
    sacc_io.sacc_to_twopoint_fits(s, str(out), n_bins=1)
...
def test_perturbed_xi_changes_output(tmp_path):
    ...
    sacc_io.sacc_to_twopoint_fits(s2, str(out2), n_bins=1)
...
def test_tomographic_sacc_raises(tmp_path):
    ...
    with pytest.raises(ValueError, match="single-bin only"):
        sacc_io.sacc_to_twopoint_fits(s, str(tmp_path / "x.fits"), n_bins=1)
