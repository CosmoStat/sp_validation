...
def create_npoint_mask(path):
    ...
    print(
        f"  ✓ nside_coverage: {npoint_map.nside_coverage}, nside_sparse: {npoint_map.nside_sparse}"
    )
...
for n_val in n_values:
    ...
    print(
        f"  ✓ nside_coverage: {mask.nside_coverage}, nside_sparse: {mask.nside_sparse}"
    )
...
npoint_gt0_area = hsp.HealSparseMap.read(f"{base_path}/coverage.hsp").get_valid_area(
    degrees=True
)
...
step_losses = [0, npoint_gt0_area - upgraded_area] + [
...
mask_areas = [
    0,
    0,
]  # npoint>0 baseline and npoint>2 cut don't have individual mask areas
for n_val in mask_order:
...
print(
    f"  Area lost by npoint>2 cut: {npoint2_loss:.2f} deg² ({(npoint2_loss / npoint_gt0_area) * 100:.1f}%)"
)
print(f"  Final area after all masks: {cumulative_areas[-1]:.2f} deg²")
print(
    f"  Total area lost from npoint>0: {total_loss:.2f} deg² ({(total_loss / npoint_gt0_area) * 100:.1f}%)"
)
