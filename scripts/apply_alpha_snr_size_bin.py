...
def parse_options(p_def):
    ...
    parser.add_option(
        ...
        type="string",
        help=("output path of the plots," + f" default='{p_def['plot_path']}'"),
    )
...
def main(argv=None):
    ...
    df_gal = pd.DataFrame(
        ...
        columns=["e1", "e2", "e1_PSF", "e2_PSF", "snr", "w", "ratio_size"],
    )
    ...
    df_gal.loc[:, "bin_R"] = pd.qcut(
        df_gal["ratio_size"], n_bins_r, labels=False, retbins=False
    )
    ...
    alpha_df = pd.DataFrame(
        ...
        columns=["R", "SNR", "alpha_1", "alpha_1_err", "alpha_2", "alpha_2_err"],
    )
    ...
    df_gal.loc[:, "e1_cor_2"] = -999.0
    df_gal.loc[:, "e2_cor_2"] = -999.0
    ...
    for i_group in range(n_bins_r * n_bins_snr):
        ...
        df_gal.loc[mask_group, "e1_cor_2"] = (
            e1_out - alpha_df.loc[i_group, "alpha_1_corr"] * e1_PSF
        )
        df_gal.loc[mask_group, "e2_cor_2"] = (
            e2_out - alpha_df.loc[i_group, "alpha_2_corr"] * e2_PSF
        )
    ...
    if params["plot_path"] is not None:
        ...
        alpha_df.loc[:, "alpha_1_corr_2"] = -999.0
        alpha_df.loc[:, "alpha_2_corr_2"] = -999.0
        alpha_df.loc[:, "alpha_1_corr_2_err"] = -999.0
        alpha_df.loc[:, "alpha_2_corr_2_err"] = -999.0
        ...
        for i_group in range(n_bins_r * n_bins_snr):
            ...
            alpha_df.loc[i_group, "alpha_1_corr_2_err"] = np.sqrt(
                res_wls.cov_params()[1, 1]
            )
            del res_wls, mod_wls
            ...
            alpha_df.loc[i_group, "alpha_2_corr__2err"] = np.sqrt(
                res_wls.cov_params()[1, 1]
            )
            del weight_out, res_wls, mod_wls
        ...
        vmin, vmax = (
            -np.max([np.abs(vmin), np.abs(vmax)]),
            np.max([np.abs(vmin), np.abs(vmax)]),
        )

        cset1 = axs[0, 0].scatter(
            SNR_bins_val, R_bins_val, c=alpha, cmap="seismic", vmin=vmin, vmax=vmax
        )
        ...
        alpha = 0.5 * (
            (alpha_df["alpha_1_corr"].values) + (alpha_df["alpha_2_corr"].values)
        )
        cset2 = axs[1, 0].scatter(
            SNR_bins_val, R_bins_val, c=alpha, cmap="seismic", vmin=vmin, vmax=vmax
        )
        ...
        alpha = 0.5 * (
            (alpha_df["alpha_1_corr_2"].values) + (alpha_df["alpha_2_corr_2"].values)
        )
        cset3 = axs[2, 0].scatter(
            SNR_bins_val, R_bins_val, c=alpha, cmap="seismic", vmin=vmin, vmax=vmax
        )
