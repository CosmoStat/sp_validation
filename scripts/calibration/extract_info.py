...
n_found, n_missing = missing_tiles(
...
print_stats(
    f"Non-overlapping objects: {n_ok:10d}, {n_ok / n_obj:10.2%}",
    stats_file,
    verbose=verbose,
)
...
n_ok = sum(cut_common)
print_stats(
    f"objects after common cut: {n_ok:10d}, {n_ok / n_obj:3.2%}",
    stats_file,
    verbose=verbose,
)
...
n_ok = sum(m_gal)
print_stats(
    f"common & ngmix = galaxy selection: {n_ok:10d}, {n_ok / n_obj:3.2%}",
    stats_file,
    verbose=verbose,
)
# -
...
# +
g_corr, g_uncorr, w, mask = get_calibrated_quantities(gal_metacal)

n_before_cuts = len(gal_metacal.ns["g1"])
n_after_cuts = len(w)
print_stats(
    f"Number of objects before cuts = {n_before_cuts}", stats_file, verbose=verbose
)
print_stats(
    f"Number of objects after cuts = {n_after_cuts}", stats_file, verbose=verbose
)
...
print_stats(
    f"effective sample size, ESS/N = {ess:.1f}/{n_gal} = {ess / n_gal:.3g}",
    stats_file,
    verbose=verbose,
)
# -
...
# Galaxies
Apix = 1  # [arcmin^2]
cbar_label = "{}, $A_{{\\rm pix}} \\approx {:.1g}$ arcmin$^2$".format(
    cbar_label_base, Apix
)
...
for comp in (0, 1):
    print_stats(
        f"dmc_{comp + 1} = {c_err[comp] / np.sqrt(n_gal):.3e}",
        stats_file,
        verbose=verbose,
    )
    print_stats(
...
for comp in (0, 1):
    if n_jack > 0:
        cjk[comp], cjk_err[comp] = jackknif_weighted_average(
            g_uncorr[comp], w, remove_size=remove_size, n_realization=n_jack
        )
...
print_stats(
    "Dispersion of complex ellipticity = {:.3f}".format(sig_eps),
    stats_file,
    verbose=verbose,
)
print_stats(
    "Dispersion of (average) single-component ellipticity = {:.3f} = {:.3f} / sqrt(2)"
    "".format(sig_eps / np.sqrt(2), sig_eps),
    stats_file,
    verbose=verbose,
)
