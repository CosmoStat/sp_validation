...
def print_all(
    results, stats_files, use_keys=None, fout=sys.stdout, header=True, all=True
):
...
def main(argv=None):
    ...
    if argv[1] == "test":
        ...
        for key in results["type"]:
            results_comb["value"][key]["comb"] = (
                results_comb["value"][key]["comb"] / results["all"][key] - 1
            ) * 100
    ...
    if argv[1] not in ["comb"]:
        ...
        print_all(
            results,
            stats_files,
            use_keys=["cw_1", "dmcw_1", "cw_2", "dmcw_2"],
            fout=f,
            all=all,
        )
        f.close()
    ...
    if argv[1] not in ["comb"]:
        ...
        if results_comb:
            print_all(
                results_comb, stats_file_comb, use_keys=use_keys, fout=f, all=False
            )
            f.close()
            col_names = [
                "m_{11}",
