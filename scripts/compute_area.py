...
def main(argv=None):
    ...
    area_deg2_eff_non_overl_tile = ufloat(
    ...
    ratio_unmasked_tile = ufloat(
        np.mean(ratio_unmasked_tot), np.std(ratio_unmasked_tot)
    )
    print(
        f"Ratio of unmasked to total pixels = {ratio_unmasked_tile:.3fP} (min = {np.min(ratio_unmasked_tot):.3f})"
    )
