...
def weighted_std(x, w):
    ...
    # print(len(x[0]))
    # print(np.average(e1, weights=w))
    # print(np.average((x - np.average(e1, weights=w))**2, weights=w))
...
def gamma_T_tc(ra_cluster, dec_cluster, ra_cat, dec_cat, e1_cat, e2_cat, w_cat = None):
    ...
    TreeCorrConfig = {'ra_units': 'degrees', 'dec_units': 'degrees',
                      'max_sep': 60, 'min_sep': 0.7, 'sep_units': 'arcminutes',
                      'nbins': 30}
...
def gamma_T_tc_spatial(cluster_cat, ra_cat, dec_cat, e1_cat, e2_cat, w_cat = None):
    ...
    for i in tqdm(range(n_cluster), total=n_cluster):
        ...
        cat_cluster = treecorr.Catalog(ra=[cluster_cat['ra'][i]], dec=[cluster_cat['dec'][i]], ra_units='degrees', dec_units='degrees')

        TreeCorrConfig = {'ra_units': 'degrees', 'dec_units': 'degrees',
                          'max_sep': 3.*R_v_ang, 'min_sep': R_v_ang/8., 'sep_units': 'arcminutes',
                          'nbins': 20}
...
def gamma_T_tc_spatial_parallel(cluster_cat, ra_cat, dec_cat, e1_cat, e2_cat, w_cat = None):
    """
    """
    ...
    def job_func(clust_ra, clust_dec, clust_z, clust_R, cat_gal, cosmo):
        ...
        cat_cluster = treecorr.Catalog(ra=[clust_ra], dec=[clust_dec], ra_units='degrees', dec_units='degrees')
        ...
        return np.array([ng.meanr*d_ang*arcmin_to_rad/clust_R, ng.meanlogr, ng.xi, ng.xi_im, ng.varxi])
    ...
    res = (Parallel(n_jobs=5, backend='loky')
          (delayed(job_func)
          (cluster_cat['ra'][i], cluster_cat['dec'][i], cluster_cat['z'][i], R_mpc[i], cat_gal, cosmo) for i in tqdm(range(n_cluster), total=n_cluster)))
...
def make_mass_map(ra, dec, g1, g2, w, proj=True):
    ...
    params = {'nside' : 2048, #healpy resolution
    ...
    kappa_E_sm = convolve_fft(kappa_E, smooth_kernel, normalize_kernel = np.sum, nan_treatment = 'fill')
    kappa_B_sm = convolve_fft(kappa_B, smooth_kernel, normalize_kernel = np.sum, nan_treatment = 'fill')
...
def stack_mm(clust_ra, clust_dec, ra, dec, g1, g2, w, output_size=50):
    ...
    # ra_pix = np.abs(ra_tmp - params['ra_min'])*step_ra
    # dec_pix = np.abs(clust_dec - params['dec_min'])*step_dec
    ra_pix = params['numbins_ra'] - np.abs(ra_tmp - params['ra_min'])*step_ra
    dec_pix = params['numbins_dec'] - np.abs(clust_dec - params['dec_min'])*step_dec
    ...
    step_ra = params['numbins_ra']/(params['ra_max'] - params['ra_min'])
    step_dec = params['numbins_dec']/(params['dec_max'] - params['dec_min'])
    # print(params['numbins_ra']/step_ra, params['numbins_dec']/step_dec)
    ...
    for ra_c, dec_c in zip(clust_ra, clust_dec):
        ...
        # ra_pix = int((ra_tmp - params['ra_min'])*step_ra)
        ra_pix = int(params['numbins_ra'] - np.abs(ra_tmp - params['ra_min'])*step_ra)
        # ra_pix = int(np.abs(ra_tmp - params['ra_min'])*step_ra)
        # dec_pix = int((dec_c - params['dec_min'])*step_dec)
        dec_pix = int(params['numbins_dec'] - np.abs(dec_c - params['dec_min'])*step_dec)
        # dec_pix = int(np.abs(dec_c - params['dec_min'])*step_dec)
        # print(ra_pix, dec_pix)
        if (ra_pix - output_size_2 < 0) or (dec_pix - output_size_2 < 0) or (ra_pix + output_size_2 > ke.shape[0]) or (dec_pix + output_size_2 > ke.shape[1]):
            # k += 1
            continue
        # print(ra_pix-25, ra_pix+25, dec_pix-25, dec_pix+25)
        # print(stacked_img.shape)
        # print(ke[ra_pix-25:ra_pix+25, dec_pix-25:dec_pix+25].shape)
        # print(ke.shape)
        stacked_img_e += ke[ra_pix-output_size_2:ra_pix+output_size_2, dec_pix-output_size_2:dec_pix+output_size_2]
        stacked_img_b += kb[ra_pix-output_size_2:ra_pix+output_size_2, dec_pix-output_size_2:dec_pix+output_size_2]
        k += 1
...
def stack_mm2(ra, dec, e1, e2, w, cluster_ra, cluster_dec, cluster_z, radius=100, n_match=100000, tree=None):
    """
    """
    ...
    k = 0
    for ra_c, dec_c, z_c in tqdm(zip(ra_clust_corr, cluster_dec, cluster_z), total=len(ra_clust_corr)):
...
def stack_mm3(ra, dec, e1, e2, w, cluster_ra, cluster_dec, cluster_z, radius=100, n_match=100000, tree=None):
    """
    """
    ...
    k = 0
    for ra_c, dec_c, z_c in tqdm(zip(xx_clust, yy_clust, cluster_z), total=len(xx_clust)):
...
#     return 5.71*(1+z)**(-0.47)*(m/M_star)**(-0.084)


def get_theo_gamT(cluster_mass, cluster_z, z_source=0.65, H0=72., Omega_m=0.3, Omega_b=0.045, delta_mdef=500, theta=None, z_distrib=None, meanbin_z=None, nz=None):
    """
    """
    ...
    def get_norm_gamT(nz, z_min, z_norm=1):
        ...
        z_mean = simps(nz[trunc_ind]*meanbin_z[trunc_ind], meanbin_z[trunc_ind])/int_nz
    ...
    def get_gt_model(m, concentration, cluster_z, z_distrib, cosmo_ccl, n_bins=500):
        ...
        integrand = gt_models * nz.reshape((n_bins, 1)) * heaviside(meanbin_z, cluster_z).reshape((n_bins, 1))
    ...
    def get_gt_model2(m, concentration, cluster_z, meanbin_z, nz, cosmo_ccl, n_bins=500):
        ...
        zz = np.arange(meanbin_z.min(), meanbin_z.max(), (meanbin_z.max()-meanbin_z.min())/n_bins)
        gt_models = []
        for i in range(n_bins):
            gt_tmp = clmm.predict_reduced_tangential_shear(R*H0/100.,
                                                             m, concentration,
            ...
            gt_models.append(gt_tmp)
        gt_models = np.array(gt_models)
        integrand = gt_models * f(zz).reshape((n_bins, 1)) * heaviside(zz, cluster_z).reshape((n_bins, 1))

        final_gt = np.array([simps(integrand[:,i], zz) for i in range(integrand.shape[1])])
        return final_gt

    arcmin_to_rad = np.pi / 180. /60.

    # cluster_concentration = c_DuttonMaccio(cluster_z, cluster_mass, h=H0/100.)
    ...
    # gt = cm.predict_reduced_tangential_shear(r3d*H0/100., mdelta=cluster_mass,
...
def get_theo_mass(cluster_z, theta, gt_profile, gt_sig, z_source, H0=72., Omega_m=0.3, Omega_b=0.045, z_distrib=None, meanbin_z=None, nz=None, delta_mdef=500):
    """
    """
    ...
    def nfw_to_shear_profile(logm, profile_info):
        ...
        m = 10.**logm
        print(m/1e14)
        # concentration = c_DuttonMaccio(cluster_z, m, h=H0/100.)
        concentration = c_XRAY(cluster_z, m, h=H0/100.)
        # gt_model = clmm.predict_reduced_tangential_shear(R*H0/100.,
        #                                                  m, concentration,
        #                                                  cluster_z, z_source, cosmo_ccl,
        #                                                  delta_mdef=200,
        #                                                  halo_profile_model='nfw')
    ...
    def get_best_m(func, args, n_guess=20, tolerence=1e-6, n_jobs=-1):
        ...
        res = (Parallel(n_jobs=n_jobs, backend='loky')
              (delayed(minimizer)
              (func, x0, args, tolerence) for x0 in tqdm(logm_0s, total=n_guess)))
        ...
        cov_value = np.max((1, np.abs(res[1][arg_min]))) * tolerence * res[2][arg_min]
    ...
    if z_distrib != None:
        logm_est, cov = get_best_m(nfw_to_shear_profile,
                                args=[R, gt_profile, gt_sig, z_distrib],
                                tolerence=1e-3)
        m_est = 10.**logm_est
        # final_model = get_gt_model(m_est, c_DuttonMaccio(cluster_z, m_est, h=H0/100.), cluster_z, z_distrib, cosmo_ccl, n_bins=100)
        ...
        logm_est, cov = get_best_m(nfw_to_shear_profile2,
                                ...
                                tolerence=1e-3)
        m_est = 10.**logm_est
        # final_model = get_gt_model2(m_est, c_DuttonMaccio(cluster_z, m_est, h=H0/100.), cluster_z, meanbin_z, nz, cosmo_ccl, n_bins=100)
...
def get_theo_xi(theta, z, nz, Omega_m=0.295, h=0.672, Omega_b=0.0516, sig8=0.7745, ns=1.044):
    """
    """
    ...
    cosmo = ccl.Cosmology(Omega_c=Omega_m-Omega_b, Omega_b=Omega_b, h=h, sigma8=sig8, n_s=ns, transfer_function='eisenstein_hu',)   


    # Create objects to represent tracers of the weak lensing signal with this
    # number density (with has_intrinsic_alignment=False)
    lens1 = ccl.WeakLensingTracer(cosmo, dndz=(z, nz))
    #lens2 = ccl.WeakLensingTracer(cosmo, dndz=(z, nz))
