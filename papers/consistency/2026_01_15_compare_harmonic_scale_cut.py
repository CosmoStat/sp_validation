...
chain_version = "v2"
assert chain_version in [f"v{i}" for i in range(5)], (
    "The chain run version is not correct."
)

# Load the output of the simulations
simulation_output = pd.read_csv(
    f"/n09data/guerrini/glass_mock_chains/summary_parameter_constraints_harmonic_space_{chain_version}.txt",
    delimiter=";",
)
...
# %%
sns.histplot(
    simulation_output["S8_MAP"], kde=True, bins=20, label="Configuration space"
)
plt.axvline(s8_fid, color="black", linestyle="--", label="Fiducial S8")
plt.legend(fontsize=12)
...
# %%
sns.histplot(
    simulation_output["a_config_MAP"], kde=True, bins=20, label="Configuration space"
)
sns.histplot(
...
plt.axvline(0.0, color="black", linestyle="--", label=r"Fiducial $A_{\rm IA}$")
plt.legend(fontsize=12)
...
# %%
sns.histplot(
    simulation_output["a_config_mean"], kde=True, bins=20, label="Configuration space"
)
sns.histplot(
...
# %%
g = sns.JointGrid(
    data=simulation_output, x="OMEGA_M_harm_MAP", y="SIGMA_8_harm_MAP", space=0
)
...
path_harmonic = (
    f"/n09data/guerrini/output_chains/{root_harmonic}/getdist_{root_harmonic}"
)
...
plt.axvline(
    delta_S8_analysis, color="red", linestyle="--", label="Difference in the analysis"
)
