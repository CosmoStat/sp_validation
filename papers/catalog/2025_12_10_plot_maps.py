...
def plot_galactic_planes(sky_map):
    ...
    planes = {
        ...
        r"$b=+10^\circ$": {"l": np.linspace(-20, 270, 1000), "b": 10, "style": "dashed"},
        r"$b=-10^\circ$": {"l": np.linspace(-20, 270, 1000), "b": -10, "style": "dashed"},
    }
    ...
    for label, params in planes.items():
        ...
        sky_map.plot(eq.ra.deg, eq.dec.deg, color="gray", linestyle=params["style"], linewidth=1.2)

def plot_healpix_map(pix_vals, nside, map_style, label, output_name, show=False, cmap=None, mask=None):
    """
    ...
    cbar = fig.colorbar(mappable, cax=cax, orientation='horizontal', fraction=0.05, pad=0.1)
...
def plot_healpix_map_ax(pix_vals, nside, map_style, label, fig, ax, cmap=None, mask=None):
    """
    Render and save a HEALPix map in an Albers equal-area projection.
...
plt.hist(kappa_E_map[~mask_footprint], bins=1000, density=True, histtype='step', label=r"$\kappa_E$")

plt.show()

# %%
hp.mollview(kappa_E_map, title=r"$\kappa_E$ map", unit=r"$\kappa_E$", cmap="coolwarm", min=-0.02, max=0.02)
plt.show()
...
shape_noise_map[~mask_shape_noise] = np.sqrt(1/2 * shape_noise_map[~mask_shape_noise] / w2_sum[~mask_shape_noise])
...
# %%
# GLASS mock maps
path_glass_mock = "/n09data/guerrini/glass_mock/results/unions_glass_sim_00109_4096.fits"
...
# %%
pix_vals_dela_e1 = weighted_mean_per_pixel(pix_star, weights_star, e1_star - e1_psf, nside)
pix_vals_dela_e2 = weighted_mean_per_pixel(pix_star, weights_star, e2_star - e2_psf, nside)
...
# %%
pix_vals_delta_t = weighted_mean_per_pixel(pix_star, weights_star, (t_star - t_psf)/t_star, nside)
