...
for i, ver in enumerate(versions):
    ...
    var_alpha = (var_tau / rho_stats[i]["rho_0_p"] ** 2) + (
        tau_stats[i]["tau_0_p"] ** 2 * var_rho / rho_stats[i]["rho_0_p"] ** 4
    )
...
for i, (ver, color, label, alpha, alpha_err) in enumerate(
    zip(versions, colors, labels, alpha_leakages, alpha_leakages_err)
):
    plt.errorbar(
        tau_stats[i]["theta"],
        alpha,
...
for i, (ver, color, label, xi_sys_i, xi_sys_err_i) in enumerate(
    zip(versions, colors, labels, xi_sys, xi_sys_err)
):
    plt.errorbar(
        tau_stats[i]["theta"],
        xi_sys_i,
