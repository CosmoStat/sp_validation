...
df_gal = pd.DataFrame(
    ...
    columns=["e1", "e2", "e1_PSF", "e2_PSF", "size_ratio", "snr", "w"],
)
...
df_gal.loc[:, "bin_R"] = pd.qcut(
    df_gal["size_ratio"], n_bins_size, labels=False, retbins=False
)
...
cset1 = ax.scatter(
    SNR_bins_val, R_bins_val, c=alpha, cmap="coolwarm", vmin=vmin, vmax=vmax
)
