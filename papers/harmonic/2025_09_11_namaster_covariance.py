...
from sp_validation.rho_tau import SquareRootScale, get_params_rho_tau
...
cl_noise, f, wsp = cv.get_sample(
    params, nside, b_lmax, b, cat_gal, n_gal, mask, unique_pix, idx_rep
)
...
plt.plot(
    ell,
    np.sqrt(
        np.diag(covar_EE_EE) / np.diag(get_cov_from_one_cov(cov_one_cov, gaussian=True))
    ),
    label="NaMaster/OneCovariance",
)
plt.xscale("squareroot")
plt.xticks(np.array([100, 400, 900, 1600]))
plt.minorticks_on()
