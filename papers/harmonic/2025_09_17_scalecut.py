...
PK = camb.get_matter_power_interpolator(
    pars, nonlinear=True, hubble_units=True, k_hunit=True, kmax=20.0, zmax=5.0
)
...
z_of_chi = interp1d(
    chi_of_z(np.linspace(0, 5, 2000)), np.linspace(0, 5, 2000), fill_value="extrapolate"
)
...
def get_lmax(k_max, alpha, k_vals, l_low=400, l_high=2048):
    ...
    def objective(ell):
        k_alpha = get_kalpha(
            np.vectorize(lambda k: Cl_integrand(k, ell, q_i, q_i))(k_vals),
            k_vals,
            alpha,
        )
        print(k_alpha)
