...
def project_cat_map(cat, e1_col, e2_col, nside=1024, w_col=None):
    ...
    unique_pix, idx, idx_rep = np.unique(
        pix_indices, return_index=True, return_inverse=True
    )
    ...
    e1_map[unique_pix] = (
        np.bincount(idx_rep, weights=weights * e1) / count_map[unique_pix]
    )
    e2_map[unique_pix] = (
        np.bincount(idx_rep, weights=weights * e2) / count_map[unique_pix]
    )
    return count_map, e1_map, e2_map


def get_field_catalog(cat, e1_col, e2_col, nside, w_col=None):
    count_map, e1_map, e2_map = project_cat_map(
        cat, e1_col, e2_col, nside=nside, w_col=w_col
    )
...
def compute_iNKA_covariance(field1, field2, coupled=False):
    ...
    if coupled:
        cl_a1b1 = get_cell(
            field1, field1, get_workspace(field1, field1), unbin=False, coupled=True
        ) / np.mean(field1.mask**2)
        cl_a1b2 = get_cell(field1, field2, wsp, unbin=False, coupled=True) / np.mean(
            field1.mask * field2.mask
        )
        cl_a2b2 = get_cell(
            field2, field2, get_workspace(field2, field2), unbin=False, coupled=True
        ) / np.mean(field2.mask**2)
    else:
    ...
    cov = nmt.gaussian_covariance(
        cw, 2, 2, 2, 2, cl_a1b1, cl_a1b2, cl_a1b2, cl_a2b2, wsp, wb=wsp
    ).reshape([n_ells, 4, n_ells, 4])
...
for i in range(n_sims):
    ...
    rho_0 = np.load(glass_root_dir + f"rho_cl_glass_mock_{index_sim}_4096.npy")[
        1:
    ].reshape((32 * 4,))
    tau_0 = np.load(glass_root_dir + f"tau_cl_glass_mock_{index_sim}_4096.npy")[
        1:
    ].reshape((32 * 4,))
...
plt.plot(
    ell_eff, np.sqrt(np.diag(cov_tau_0_corrected[:, 0, :, 0])), label="iNKA corrected"
)
...
plt.errorbar(
    ell_eff,
    ell_eff * rho_cl[0],
    yerr=ell_eff * np.sqrt(np.abs(cov_rho_0_ee.diagonal())),
    label=r"$\rho_0$ EE",
    fmt="o",
    capsize=2,
)
plt.errorbar(
    ell_eff,
    ell_eff * rho_cl[3],
    yerr=ell_eff * np.sqrt(np.abs(cov_rho_0_bb.diagonal())),
    label=r"$\rho_0$ BB",
    fmt="o",
    capsize=2,
)
...
plt.errorbar(
    list_offset[0],
    ell_eff * tau_cl[0],
    yerr=ell_eff * np.sqrt(cov_tau_0_ee.diagonal()),
    label=r"$\tau_0$ EE",
    fmt="o",
    capsize=2,
)
plt.errorbar(
    list_offset[1],
    ell_eff * tau_cl_corrected[0],
    yerr=ell_eff * np.sqrt(cov_tau_0_ee.diagonal()),
    label=r"$\tau_0$ corrected EE",
    fmt="o",
    capsize=2,
)
plt.errorbar(
    list_offset[2],
    ell_eff * tau_cl[3],
    yerr=ell_eff * np.sqrt(cov_tau_0_bb.diagonal()),
    label=r"$\tau_0$ BB",
    fmt="o",
    capsize=2,
)
plt.errorbar(
    list_offset[3],
    ell_eff * tau_cl_corrected[3],
    yerr=ell_eff * np.sqrt(cov_tau_0_bb.diagonal()),
    label=r"$\tau_0$ corrected BB",
    fmt="o",
    capsize=2,
)
...
tau_cl_samples = np.random.multivariate_normal(
    mean=tau_cl[0], cov=cov_tau_0_ee, size=10000
)
tau_cl_corrected_samples = np.random.multivariate_normal(
    mean=tau_cl_corrected[0], cov=cov_tau_0_ee, size=10000
)
...
plt.errorbar(
    ...
    tau_cl[0] / rho_cl[0],
    yerr=alpha_ell_std,
    label="Without object-wise leakage correction",
    fmt="o",
    capsize=2,
)
plt.errorbar(
    list_offset[1],
    tau_cl_corrected[0] / rho_cl[0],
    yerr=alpha_ell_corrected_std,
    label="With object-wise leakage correction",
    fmt="o",
    capsize=2,
)
...
plt.plot(
    ell_eff, ell_eff * tau_cl[0] ** 2 / rho_cl[0], label="Leakage bias uncorrected"
)
plt.plot(ell_eff, ell_eff * cell_cl_corrected[0], label=r"$C^{EE}_\ell$ corrected")
plt.plot(
    ell_eff,
    ell_eff * tau_cl_corrected[0] ** 2 / rho_cl[0],
    label="Leakage bias corrected",
)
...
plt.plot(
    ell_eff,
    cell_cl_corrected[0] / cell_cl[0] - 1,
    label=r"Ratio corrected/uncorrected EE",
)
plt.plot(
    ell_eff,
    cell_cl_corrected[3] / cell_cl[3] - 1,
    label=r"Ratio corrected/uncorrected BB",
)
...
plt.errorbar(
    list_offset[0],
    leakage_bias / cell_cl[0],
    yerr=leakage_bias_std / cell_cl[0],
    label=r"Without object-wise leakage correction",
    fmt="o",
    markersize=3,
    capsize=2,
)
plt.errorbar(
    list_offset[1],
    leakage_bias_corrected / cell_cl_corrected[0],
    yerr=leakage_bias_corrected_std / cell_cl_corrected[0],
    label="With object-wise leakage correction",
    fmt="o",
    markersize=3,
    capsize=2,
...
tau_samples_bb = np.random.multivariate_normal(
    mean=tau_cl[3], cov=cov_tau_0_bb, size=10000
)
tau_corrected_samples_bb = np.random.multivariate_normal(
    mean=tau_cl_corrected[3], cov=cov_tau_0_bb, size=10000
)
...
plt.errorbar(
    list_offset[1],
    leakage_bias_corrected,
    yerr=leakage_bias_corrected_std_bb,
    label="With object-wise leakage correction",
    fmt="o",
    markersize=3,
    capsize=2,
...
plt.plot(ell_eff, ell_eff * cell_cl[3], label=r"$C^{BB}_\ell$ uncorrected")
plt.plot(
    ell_eff, ell_eff * tau_cl[3] ** 2 / rho_cl[3], label="Leakage bias uncorrected"
)
plt.plot(ell_eff, ell_eff * cell_cl_corrected[3], label=r"$C^{BB}_\ell$ corrected")
plt.plot(
    ell_eff,
    ell_eff * tau_cl_corrected[3] ** 2 / rho_cl[3],
    label="Leakage bias corrected",
)
...
pseudo_cl = fits.getdata(
    "/home/guerrini/sp_validation/cosmo_val/output/pseudo_cl_SP_v1.4.5_leak_corr.fits"
)
cov_cl = fits.open(
    "/home/guerrini/sp_validation/cosmo_val/output/pseudo_cl_cov_SP_v1.4.5_leak_corr.fits"
)
...
plt.errorbar(
    ell_eff,
    ell_eff * pseudo_cl["EE"],
    yerr=ell_eff * np.sqrt(np.diag(cov_cl["COVAR_EE_EE"].data)),
    label=r"$C^{EE}_\ell$ SP_v1.4.5 corrected",
    fmt="o",
    markersize=3,
    capsize=2,
)
...
plt.errorbar(
    ell_eff,
    ell_eff * pseudo_cl["BB"],
    yerr=ell_eff * np.sqrt(np.diag(cov_cl["COVAR_BB_BB"].data)),
    label=r"$C^{BB}_\ell$ SP_v1.4.5 corrected",
    fmt="o",
    markersize=3,
    capsize=2,
)
