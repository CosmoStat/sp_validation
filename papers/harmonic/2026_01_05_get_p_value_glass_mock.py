...
roots_glass_mock = [
    f"glass_mock_{chain_version}_{str(i).zfill(5)}" for i in range(1, max_sim + 1)
]
...
# %%
def get_chi2_glass_mock(
    root, root_glass_chains, root_glass_dv, lower_bound, upper_bound
):
    # Read the theory prediction at best fit
    ell = np.loadtxt(
        f"{root_glass_chains}/{root}/best_fit/{root}_cell_{best_fit_method}/shear_cl/ell.txt"
    )
    shear_cl = np.loadtxt(
        f"{root_glass_chains}/{root}/best_fit/{root}_cell_{best_fit_method}/shear_cl/bin_1_1.txt"
    )
    ...
    data = fits.open(
        f"{root_glass_dv}/glass_mock_{idx}/cosmosis_glass_mock_{chain_version_data}_{idx}.fits"
    )
...
for i, root in enumerate(roots_glass_mock):
    if i + 1 in failed_simulations:
        continue
    metrics[root] = {}
    metrics[root]["chi2"] = get_chi2_glass_mock(
        root, root_glass_chains, root_glass_dv, lower_bound_cell_ee, upper_bound_cell_ee
    )
    metrics[root]["chi2_map"] = get_chi2_map_glass_mock(root, root_glass_chains)

# %%
chi2_glass_mocks = np.array(
    [
        metrics[root]["chi2"]
        for i, root in enumerate(roots_glass_mock)
        if i + 1 not in failed_simulations
    ]
)
chi2_glass_mocks_map = np.array(
    [
        metrics[root]["chi2_map"]
        for i, root in enumerate(roots_glass_mock)
        if i + 1 not in failed_simulations
    ]
)
...
sns.histplot(
    chi2_glass_mocks, bins=25, kde=False, stat="density", label=best_fit_method
)

sns.histplot(chi2_glass_mocks_map, bins=25, kde=False, stat="density", label="MLE")
...
def plot_glass_mock_fit_root(root, root_glass_chains, root_glass_dv):
    ...
    plt.plot(ell, ell * shear_cl, label="Best-fit model", c="r")
...
first_run = np.load(f"{path_first_run}/cl_glass_mock_{idx_sim}_4096.npy")
second_run = np.load(f"{path_second_run}/cl_glass_mock_{idx_sim}_4096.npy")

chain_version_data = "v0" if chain_version in ["v0", "v1"] else "v2"
data = fits.open(
    f"{root_glass_dv}/glass_mock_{idx_sim}/cosmosis_glass_mock_{chain_version_data}_{idx_sim}.fits"
)
...
mask = (dv_fiducial["CELL_EE"].data["ANG"] > low_ell_cut) & (
    dv_fiducial["CELL_EE"].data["ANG"] < high_ell_cut
)
...
chi2_fiducial = np.dot(
    (cell_bf - cell_fiducial),
    np.dot(np.linalg.inv(cov_fiducial), (cell_bf - cell_fiducial)),
)
...
pte_string = (
    rf"${{\rm PTE}} = {mantissa:.2f} \times 10^{{{exponent}}}$"
    if exponent != 0
    else rf"${{\rm PTE}} = {p_value:.2f}$"
)
...
plt.text(x_text, y_text, chi2_string, fontsize=12)
