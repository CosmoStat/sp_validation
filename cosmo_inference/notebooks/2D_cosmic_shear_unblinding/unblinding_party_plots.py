...
display(Markdown("## 🎉 Let the Unblinding Party begin 🎉"))
# %%
# 1. Plot the datavectors without best-fit
display(Markdown("### 1.a. Plot the datavectors without best-fit"))
...
display(Markdown(r"### 1.b. Plot the datavectors without best-fit ($\xi_\pm$)"))
...
display(Markdown("### 2. Plot the best-fit datavectors"))
...
labels = [r"UNIONS $C_\ell$", r"UNIONS $\xi_\pm(\vartheta)$"]
...
properties = utils.update_properties_w_roots(
...
xi_data_path = os.path.join(
    path_datavectors,
    f"SP_v1.4.6.3_config/SP_v1.4.6.3_{blind}/cosmosis_{fiducial_root_xi_data}.fits",
)
utils.plot_best_fit_config(
    xi_data_path,
    root_to_plot,
    path_output_chains,
    line_args,
    ...
    labels=labels,
    loc_legend=loc_legend,
    bbox_to_anchor_xip=bbox_to_anchor_xip,
    bbox_to_anchor_xim=bbox_to_anchor_xim,
    properties=properties,
)

# %%
# 3. Do a whisker plot with external experiments and our constraints
display(Markdown("### 🥸 Time to look at the whisker plot 🥸"))
...
for bl in ["A", "B", "C"]:
    if bl != blind:
        ...
        roots.append(f"SP_v1.4.6.3_{bl}_fiducial_config")
        legend_labels.append(rf"UNIONS $C_\ell$, Blind {bl}")
        legend_labels.append(rf"UNIONS $\xi_\pm(\vartheta)$, Blind {bl}")
    ...
    if category != "external":
        if category == "configuration":
            path_samples = os.path.join(
                path_output_chains, f"{root}/samples_{root}.txt"
            )
            path_getdist = os.path.join(path_output_chains, f"{root}/getdist_{root}")
        elif category == "harmonic":
            path_samples = os.path.join(
                path_output_chains, f"{root}/{root}/samples_{root}_cell.txt"
            )
            path_getdist = os.path.join(
                path_output_chains, f"{root}/{root}/getdist_{root}"
            )
        elif category == "external_compute_sample":
            path_samples = os.path.join(
                path_output_chains, f"ext_data/{root}/samples_{root}.txt"
            )
            path_getdist = os.path.join(
                path_output_chains, f"ext_data/{root}/getdist_{root}"
            )
        else:
...
axs[0].text(0.25, 0.05, r"(i)", fontsize=14, fontweight="bold", va="center", ha="right")
...
line_args = [dict(color=col, ls="solid") for col in colours]
