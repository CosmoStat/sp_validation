...
def export_power_spectra_for_cosmocov(analysis_results: Dict, output_dir: str, 
    ...
    for key, data in power_spectra.items():
        ...
        header = (f"# UNIONS mask power spectrum for nside={nside}\n"
                 f"# Generated by analyze_mask_power_spectrum.py on 2025-08-18\n"
                 f"# Format: ell C_ell\n"
                 f"# lmax = {len(cl)-1}")
...
def main():
    ...
    if hasattr(sys, 'ps1'):
        ...
        snakemake = snakemake_interactive(
            output_dir="/n17data/cdaley/unions/pure_eb/output/masks", 
            config_path="/n17data/cdaley/unions/pure_eb/config.yaml"
        )
    else:
