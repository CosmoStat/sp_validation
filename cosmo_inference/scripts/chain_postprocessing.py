...
def compute_limits(chain, param_name):
    ...
    param_stats = margestats.parWithName(param_name)
    return (
        param_stats.limits[0].upper,
        param_stats.limits[0].lower,
        param_stats.limits[1].upper,
        param_stats.limits[1].lower,
    )


def load_samples_and_write_paramnames(
    path_samples, path_paramnames, chain_type="polychord"
):
    """
    ...
    with open(path_paramnames, "w") as file:
        for i in range(len(params)):
            if len(params[i].split("--")) > 1:
                ...
                if "Legacy" in path_paramnames and param_name not in [
                    "OMEGA_M",
                    "SIGMA_8",
                ]:
                    continue
...
def load_chain(path_gd, smoothing_scale=0.3):
    ...
    chain = g.samples_for_root(
        ...
        settings={
            "ignore_rows": 0,
            "smooth_scale_1D": smoothing_scale,
            "smooth_scale_2D": smoothing_scale,
        },
    )
    return chain
    ...
    for i, par in enumerate(likestats.names):
        if best_fit_method == "weighted_mean":
            ...
            if par.name == "S_8" or par.name == "s_8_input" or par.name == "OMEGA_M":
                ...
                best_fit = (
                    s8_map_2D
                    if par.name == "S_8" or par.name == "s_8_input"
                    else omega_m_map_2D
                )
            else:
                best_fit = compute_map_1D(chain, par.name)
        ...
        best_fit_params.update({par.name: best_fit})
    return best_fit_params


def compute_best_fit(
    path_ini_files, best_fit, root, is_harmonic, blind=None, ini_file_root=None
):
    ...
    if ini_file_root is None:
        ...
        ini_file_root = os.path.join(
            path_ini_files, subdir, f"cosmosis_pipeline_{root}_cell.ini"
        )
    ...
    result = subprocess.run(
        ["cosmosis", ini_file_root], env=env, capture_output=True, text=True
    )
    print(f"STDOUT:\n{result.stdout}")
    print(f"STDERR:\n{result.stderr}")
...
def compute_best_fit_xi_from_cell(output_folder, root, best_fit_params, theta_rad):
    ...
    np.savetxt(
        output_folder + "{}/best_fit/shear_xi_plus/bin_1_1.txt".format(root), xi_p
    )
    np.savetxt(
        output_folder + "{}/best_fit/shear_xi_minus/bin_1_1.txt".format(root), xi_m
    )
    np.savetxt(
        output_folder + "{}/best_fit/shear_xi_plus/theta.txt".format(root), theta_rad
    )
    np.savetxt(
        output_folder + "{}/best_fit/shear_xi_minus/theta.txt".format(root), theta_rad
    )

    print(
        f"Best fit xi+ and xi- from Cl's computed and saved in {output_folder + '{}/best_fit/shear_xi_plus'.format(root)} and {output_folder + '{}/best_fit/shear_xi_minus'.format(root)}"
    )
...
def update_properties_w_roots(
    ...
    try:
        ...
        properties[root] = {"lower_bound_cell_ee": 0.0, "upper_bound_cell_ee": 2048}
...
def plot_best_fit(
    ...
    labels=None,
    properties=None,
    paths_to_bestfit=None,
):
    data = fits.open(
        f"/home/guerrini/sp_validation/cosmo_inference/data/{data_points}/cosmosis_{data_points}.fits"
    )
    ...
    for idx, (label, root) in enumerate(zip(labels, root_to_plot)):
        ...
        if paths_to_bestfit is None:
            ...
            shear_cl = np.loadtxt(
                paths_to_bestfit[idx] + "best_fit/shear_cl/bin_1_1.txt"
            )
...
def plot_best_fit_config(
    ...
    multiply_theta=True,
    loc_legend="best",
    bbox_to_anchor_xip=None,
    bbox_to_anchor_xim=None,
    label_data="Fiducial data",
    labels=None,
    properties=None,
    paths_to_bestfit=None,
    ...
    for idx, (label, root) in enumerate(zip(labels, root_to_plot)):
        ...
        if paths_to_bestfit is None:
            theta = (
                (
                    np.loadtxt(
                        output_folder
                        + "{}/best_fit/shear_xi_plus/theta.txt".format(root)
                    )
                )
                * 180
                / np.pi
            ...
            theta = (
                (np.loadtxt(paths_to_bestfit[idx] + "best_fit/shear_xi_plus/theta.txt"))
                * 180
                / np.pi
                * 60
            )
            xi_plus = np.loadtxt(
                paths_to_bestfit[idx] + "best_fit/shear_xi_plus/bin_1_1.txt"
            )
            xi_minus = np.loadtxt(
        ...
        ax.plot(
            theta,
            theta * xi_plus[mask] if multiply_theta else xi_plus[mask],
            label=label,
            **line_args[idx],
        )
        ax2.plot(
            theta,
            theta * xi_minus[mask] if multiply_theta else xi_minus[mask],
            label=label,
            **line_args[idx],
        )
