...
1. **Idempotence / plumbing (zero-shift).** Blinding with a zero shift (hidden cosmology equal to fiducial) reproduces the input file's ξ±, COSEBIs, and pure-E/B values exactly. This exercises the extract/merge/re-derivation plumbing only — a broken `theory_fn` also passes, since `theory_fn(fiducial) − theory_fn(fiducial) = 0` for any callable. The backend itself is tested by AC #3 (an independently written direct-CCL reference; AC #2 shares the backend and so cannot catch a wrong one).
2. **On-file shift equals the recovered theory difference (fork-draw recovery + merge placement).** For a fixed non-zero seed on a two-bin fixture, the per-row shift actually present in the blinded file (`blinded.mean − true.mean` at each shifted block's rows) equals `theory_fn(hidden_params) − theory_fn(fiducial_params)` — where `hidden_params` is recovered by re-running the fork's draw on that seed — to ~1e-10. This confirms the fork's draw is reproducible from `(seed, config)` and the merge places the shift at the right rows. It is **not** a backend-correctness test: both sides run the same `theory_fn`, so a wrong cosmology dependence cancels and a wrong backend still passes here — backend correctness is carried by AC #3, which compares the realized shift against an independently written direct-CCL reference. Run for all three blocks.
...
9. **Pure-EB NaN-parity under the blind.** Re-derived pure-E/B follows the pipeline's edge-based integration bounds; the Schneider estimator returns NaN wherever a reporting point lacks interior support against those bounds. The test asserts the re-derived NaN pattern is **identical on the true and blinded files** — blinding is a pure shift of the estimator's inputs, so it moves the finite values but never moves a NaN. On production files (and on the fixture, which now seeds pure-E/B through the exact pipeline convention — measured coarse reporting ξ± + fine integration grid + edge-based bounds) the reporting grid is a strict sub-range of the integration range, so the degeneracy does not fire and the pattern is empty; the invariant the AC enforces is the parity, which holds whether or not any point is degenerate.
